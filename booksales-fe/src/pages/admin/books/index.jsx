...
export default function AdminBooks() {
  ...
  const handleDelete = async (id) => {
    const confirmDelete = window.confirm("Are youe sure you want to delete this book?");
              ...
              <tbody>
                ...
                {filteredBooks.length > 0 ?
                  filteredBooks.map((book) => (
                    <tr key={book.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <th
                        scope="row"
                      ...
                      <td className="px-4 py-3">Rp{Number(book.price || 0).toLocaleString("id-ID")}</td>
                      <td className="px-4 py-3">{book.stock}</td>
                      <td className="px-4 py-3">
                        <img 
                          src={book.cover_photo ? `http://localhost:8000/storage/books/${book.cover_photo}?t=${new Date().getTime()}` : "https://via.placeholder.com/100x150?text=No+Cover"}
                          alt={book.title}
                          className="w-12 h-16 object-cover rounded shadow-sm border border-gray-200"
                          onError={(e) => { e.target.onerror = null; e.target.src = "https://via.placeholder.com/100x150?text=Error" }}
                        />
                      </td>
